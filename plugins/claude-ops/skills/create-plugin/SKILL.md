---
name: create-plugin
description: Design, scaffold, review, and package Claude plugins with skills, commands, agents, hooks, MCP configs, manifests, and distribution notes.
---

# Create Claude Plugin

Build a Claude plugin from scratch through guided conversation. Walk the user through discovery, planning, design, implementation, review, and packaging for chat, Desktop, Cowork, or organization marketplace distribution.

## Overview

## Current distribution note

For Team and Enterprise migration, prefer organization-managed plugin marketplaces when a bundle should be assigned to a specific group. Use standalone skill provisioning when one skill should be available broadly. Package plugins with `.claude-plugin/plugin.json` at the plugin root and skills under `skills/<skill-name>/`.
...
This schema is shared with Claude Code's plugin system, but you're creating a Claude plugin for chat, Desktop, Cowork, or organization marketplace distribution.
Claude users will usually find commands and skills the most useful.
...
> You can mention this is an option if it seems like the user wants to distribute the plugin externally, but do not proactively ask about this with the available structured-question interface.
...
When you ask the user something, use the available structured-question interface. Don't assume "industry standard" defaults are correct. Note: the available structured-question interface always includes a Skip button and a free-text input box for custom answers, so do not include `None` or `Other` as options.
