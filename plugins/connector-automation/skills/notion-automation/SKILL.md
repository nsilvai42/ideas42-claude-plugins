...
description: Use a configured Notion automation connector to search, create, update, and organize Notion workspace content.
---

## MANUAL MIGRATION REQUIRED

This skill depends on Rube MCP tools. Codex does not enforce the original Claude `requires` metadata, so confirm a `rube` MCP server and an active Notion toolkit connection before using it.
