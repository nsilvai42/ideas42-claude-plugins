...
description: Use a configured Figma automation connector to create, inspect, and update Figma design artifacts.
---

## MANUAL MIGRATION REQUIRED

This skill depends on Rube MCP tools. Codex does not enforce the original Claude `requires` metadata, so confirm a `rube` MCP server and an active Figma toolkit connection before using it.
