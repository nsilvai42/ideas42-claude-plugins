...
description: Use a configured Miro automation connector to create, inspect, and update boards, diagrams, and collaboration artifacts.
---

## MANUAL MIGRATION REQUIRED

This skill depends on Rube MCP tools. Codex does not enforce the original Claude `requires` metadata, so confirm a `rube` MCP server and an active Miro toolkit connection before using it.
