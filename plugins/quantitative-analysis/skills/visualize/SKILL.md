---
name: visualize
description: Create self-contained HTML visuals, decks, dashboards, diagrams, timelines, infographics, reports, and interactive explanation artifacts.
---
...
Deliver the result in the lowest-friction way the current Claude environment supports:

1. If a workspace file system and browser opener are available, save the HTML file, open it locally, and return the absolute path.
2. If running in Claude web, Enterprise chat, or another environment without local browser access, create or attach the HTML artifact and tell the user where it is available.
3. If neither file creation nor artifacts are available, provide a single fenced HTML block and name the intended filename.

Do not assume `open <filename>.html`, `xdg-open`, or `file://` links will work. Use them only after confirming the environment supports local files.
