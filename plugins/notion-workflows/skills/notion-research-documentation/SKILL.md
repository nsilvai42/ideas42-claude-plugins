...
description: Search Notion, synthesize related pages, cite sources, and create structured research briefs, summaries, reports, and comparisons.
---
...
1. **Search for relevant content**: Use `the available Notion search tool` to find pages
2. **Fetch detailed information**: Use `the available Notion fetch/read tool` to read full page content
3. **Synthesize findings**: Analyze and combine information from multiple sources
4. **Create structured output**: Use `the available Notion page-creation tool` to write documentation
...
```
Use the available Notion search tool with the research topic
...
```
Use the available Notion fetch/read tool for each relevant page URL
