...
description: Prepare Notion-backed meeting pre-reads, agendas, context briefs, attendee notes, open questions, and follow-up docs.
---
...
1. **Gather Notion context**: Use `the available Notion search tool` to find related pages
2. **Fetch details**: Use `the available Notion fetch/read tool` to read relevant content
3. **Enrich with research**: Use Claude's knowledge to add context, industry insights, or best practices
4. **Create internal pre-read**: Use `the available Notion page-creation tool` for background context document (for attendees)
5. **Create external agenda**: Use `the available Notion page-creation tool` for meeting agenda (shared with all participants)
...
```
Use the available Notion search tool to find:
...
1. Fetch with the available Notion fetch/read tool
...
```
Use the available Notion page-creation tool for internal doc:
...
```
Use the available Notion page-creation tool for meeting doc:
