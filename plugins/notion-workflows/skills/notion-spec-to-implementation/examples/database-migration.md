...
### 1. Find & Fetch Spec
```
Notion search connector → Found "User Preferences Schema Migration Spec"
Notion fetch/read connector → Extracted requirements
```
...
```
Notion page-creation connector
pages: [{
  properties: {
...
```
Notion search connector → Found "Engineering Tasks" database
Notion fetch/read connector → Got schema (Task, Status, Priority, Assignee, etc.)
