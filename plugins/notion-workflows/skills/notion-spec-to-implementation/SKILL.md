...
description: Turn Notion product or technical specs into implementation plans, task breakdowns, acceptance criteria, milestones, and progress updates.
---
...
Use Notion connector tools when they are enabled. If a Notion connector is unavailable, ask the user for page/database links or produce copy-ready Notion content that they can paste manually. Do not invent Notion page IDs, database IDs, or tool names.
...
1. **Find spec**: Use `the available Notion search tool` to locate specification page
2. **Fetch spec**: Use `the available Notion fetch/read tool` to read specification content
3. **Extract requirements**: Parse and structure requirements from spec
4. **Create plan**: Use `the available Notion page-creation tool` for implementation plan
5. **Find task database**: Use `the available Notion search tool` to locate tasks database
6. **Create tasks**: Use `the available Notion page-creation tool` for individual tasks in task database
7. **Track progress**: Use `the available Notion page-update tool` to log progress and update status
...
```
1. Search for spec:
   - Use the available Notion search tool with spec name or topic
...
```
1. Fetch spec page:
   - Use the available Notion fetch/read tool with spec URL/ID from search results
...
```
Use the available Notion page-creation tool:
...
1. Search for task database:
   - Use the available Notion search tool to find "Tasks" or "Task Management" database
...
2. Fetch database schema:
   - Use the available Notion fetch/read tool with database URL/ID
...
1. Create task in task database using the available Notion page-creation tool
...
**"Can't find spec"**: Use the available Notion search tool with spec name/topic, try broader terms, or ask user for URL
