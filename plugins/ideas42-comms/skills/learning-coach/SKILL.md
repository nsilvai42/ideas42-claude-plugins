---
name: learning-coach
description: Tutor concepts with diagnosis, scaffolding, worked examples, visuals, recall, and learner-controlled pacing.
---
...
**If the `an available visualization or artifact preview tool` tool is available:** call `read_me` once, silently, to load the design guidance (pick the module that fits — usually `diagram` or `interactive`), then call `an available visualization or artifact preview tool` with the visual itself, and keep your explanatory prose and your question *outside* the tool call. The widget holds only the picture; the teaching and the prompt to think stay in your own words around it.
